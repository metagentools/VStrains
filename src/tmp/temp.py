...
def graph_grouping(
    graph: Graph, simp_node_dict: dict, forward, reverse, partition_length_cut_off
):
    """
    ...
    if not src_is_connect or not sink_is_connect:
        ...
        for v in sorted(circuit_nodes, key=lambda n: graph.vp.dp[n], reverse=True):
            ...
            if graph.vp.id[v] in contig_node_set:
                print(
                    "already in the contig set, never consider to break the relevant edge"
                )
                continue
...
def graph_reduction(
    ...
    simp_node_dict: dict,
    simp_edge_dict: dict,
    node_to_contig_dict: dict,
    edge_to_contig_dict: dict,
    output_file,
    min_cov,
    min_len,
    overlap,
    ...
    for cno, (contig, clen, ccov) in contig_dict.items():
        if clen >= min_len:
            ...
            contig_reduction(
                graph, contig, cno, clen, ccov, simp_node_dict, simp_edge_dict, min_cov
            )
        else:
...
def assign_edge_flow(
    ...
    simp_node_dict: dict,
    simp_edge_dict: dict,
    consistent_node: dict,
    inconsistent_node: dict,
):
    """
        ...
        for no, node in simp_node_dict.items():
            ...
            if in_d == 1:
                for e in in_e:
                    if (
                        graph.ep.flow[e] == 0.0
                        and (graph.vp.id[e.source()], no) in simp_edge_dict
                    ):
                        if in_w <= 0:
                            print(
                                "in low edge flow error: ",
            ...
            out_d = len(
                [n for n in node.out_neighbors() if graph.vp.color[n] == "black"]
            )
            node_out_edges = [
            ...
            if out_d == 1:
                for e in out_e:
                    if (
                        graph.ep.flow[e] == 0.0
                        and (no, graph.vp.id[e.target()]) in simp_edge_dict
                    ):
                        if out_w <= 0:
                            print_vertex(graph, node, "Current node")
                            print(
                                "out low edge flow error: ",
                                graph.vp.id[e.source()],
                                " -> ",
                                graph.vp.id[e.target()],
                                out_w,
    ...
    for (u, v), e in simp_edge_dict.items():
        if graph.ep.flow[e] == 0.0:
            ...
            u_degree = len(
                [n for n in u.out_neighbors() if graph.vp.color[n] == "black"]
            )
            ...
            v_degree = len(
                [n for n in v.in_neighbors() if graph.vp.color[n] == "black"]
            )
            ...
            if u_flow_remain == 0 or v_flow_remain == 0:
                ...
                assign_flow = (
                    (u_flow_remain / u_degree) + (v_flow_remain / v_degree)
                ) / 2
            if assign_flow <= 0:
                print(
                    "low edge flow error: ",
...
def node_dp_rebalance(graph: Graph, simp_node_dict: dict, simp_edge_dict: dict):
    ...
    while set(dp_dict):
        ...
        while fifo_queue:
            ...
            if DEBUG_MODE:
                print(
                    "Node: {0} dp from {1} to {2}".format(
                        graph.vp.id[u], u_dp, round(max_dp)
                    )
                )
            ...
            for adj_node in u.all_neighbors():
                if (
                    graph.vp.visited[adj_node] == -1
                    and graph.vp.color[adj_node] == "black"
                ):
    ...
    print("Final refine")
    assign_edge_flow(
        graph, simp_node_dict, simp_edge_dict, consistent_node, inconsistent_node
    )
    ...
    queue = []
    queue.append(
        (sorted(simp_node_dict.values(), key=lambda n: graph.vp.dp[n], reverse=True))[0]
    )
    visited = []
    while queue:
        node = queue.pop()
        ...
        graph.vp.dp[node] = numpy.max(
            [curr_dp, in_neighbor_dp_sum, out_neighbor_dp_sum, in_eflows, out_eflows]
        )
        print(
            graph.vp.id[node],
            " node depth from {0} to {1}".format(curr_dp, graph.vp.dp[node]),
        )
    ...
    for no, node in simp_node_dict.items():
        ...
        if in_d == 1:
            for e in node_in_edges:
                src = e.source()
                if src.out_degree() == 1:
                    graph.ep.flow[e] = numpy.max(
                        [w, graph.ep.flow[e], graph.vp.dp[src]]
                    )
                    un_assigned_edge = un_assigned_edge - 1
        ...
        if out_d == 1:
            for e in node_out_edges:
                ...
                if tgt.in_degree() == 1:
                    graph.ep.flow[e] = numpy.max(
                        [w, graph.ep.flow[e], graph.vp.dp[tgt]]
                    )
                    un_assigned_edge = un_assigned_edge - 1
...
def graph_split_by_breadth(graph: Graph, simp_node_dict: dict, simp_edge_dict: dict):
    ...
    while queue:
        ...
        graph.vp.dp[curr_node] = numpy.max(
            [curr_dp, in_neighbor_dp_sum, out_neighbor_dp_sum, in_eflows, out_eflows]
        )
        print(
            graph.vp.id[curr_node],
            " node depth from {0} to {1}".format(curr_dp, graph.vp.dp[curr_node]),
        )
...
def coverage_rebalance(
    graph: Graph, simp_node_dict: dict, simp_edge_dict: dict, min_cov, cutoff=125
):
    """
        ...
        for edge in curr_node.all_edges():
            ...
            if u in inconsistent_node and v in inconsistent_node:
                ...
                for incon_edge in incon_edges:
                    ...
                    maxflow = max(
                        graph.ep.flow[incon_edge],
                        round((graph.vp.dp[tgt] / all_incon_dp) * out_flow_remain, 2),
                    )
                ...
                for incon_edge in incon_edges:
                    ...
                    maxflow = numpy.max(
                        [
                            graph.ep.flow[incon_edge],
                            round(
                                (graph.vp.dp[src] / all_incon_dp) * in_flow_remain, 2
                            ),
                        ]
                    )
                    if maxflow != graph.ep.flow[incon_edge]:
        ...
        if curr_id in consistent_node:
            ...
            if len(node_in_edges) < 1:
                ...
                for e in node_in_edges:
                    src = e.source()
                    maxflow = numpy.max(
                        [graph.ep.flow[e], round((graph.vp.dp[src] / total_dp) * w)]
                    )
                    if graph.ep.flow[e] != maxflow:
                        graph.ep.flow[e] = maxflow
                        no_changes = False
            ...
            if len(node_out_edges) < 1:
                ...
                # mutliple in edges
                total_dp = numpy.sum(
                    [graph.vp.dp[u] for u in curr_node.out_neighbors()]
                )
                for e in node_out_edges:
                    tgt = e.target()
                    maxflow = numpy.max(
                        [graph.ep.flow[e], round((graph.vp.dp[tgt] / total_dp) * w)]
                    )
                    if graph.ep.flow[e] != maxflow:
                        graph.ep.flow[e] = maxflow
                        no_changes = False
    ...
    print(
        "Total consistent node after most confident extraction: ",
        consistent_count,
        len(simp_node_dict),
    )
    ...
    for no, node in list(untracked_node.items()):
        ...
        if node_expansion_both(graph, consistent_node, inconsistent_node, node):
            ...
            inconsistent_node[no] = node
    print(
        "Total consistent node after untracked classification: ",
        consistent_count,
        len(simp_node_dict),
    )
    ...
    sorted_consistent_node = sorted(
        consistent_node.items(), key=lambda x: graph.vp.dp[x[1]], reverse=True
    )
            ...
            if curr_id in inconsistent_node:
                ...
                if fix:
                    ...
                    if DEBUG_MODE:
                        print(
                            curr_id,
                            "inconsistent node depth from {0} to {1}".format(
                                curr_dp, graph.vp.dp[curr_node]
                            ),
                        )
                ...
                in_eflows = numpy.sum([graph.ep.flow[e] for e in curr_node.in_edges()])
                out_eflows = numpy.sum(
                    [graph.ep.flow[e] for e in curr_node.out_edges()]
                )
                maxdp = numpy.max(
                    [
                        curr_dp,
    ...
    print(
        "Total consistent node after solid fix: ", consistent_count, len(simp_node_dict)
    )
    print([int(n) for n in consistent_node])
    print([int(n) for n in inconsistent_node])
    print([int(n) for n in untracked_node])
...
def contig_merge(
    ...
    simp_node_dict: dict,
    simp_edge_dict: dict,
    temp_contigs_dict: dict,
    node_to_contig_dict: dict,
    edge_to_contig_dict: dict,
    node_usage_dict: dict,
    output_file,
    min_cov,
    min_len,
    max_len,
        ...
        for head_cno, [head_contig, head_clen, head_ccov] in temp_contigs_dict.items():
            ...
            if s_path != None:
                ...
                print(
                    ...
                    s_len,
                    "path: ",
                    [int(v) for v in s_path_ids],
                )
                s_path_edge_flow = contig_flow(graph, simp_edge_dict, s_path_ids)
                s_path_ccov = (
                    numpy.mean(s_path_edge_flow) if len(s_path_edge_flow) != 0 else 0
                )
                dist_matrix[(tail_cno, head_cno)] = (s_path_ids, s_len, s_path_ccov)

                concat_ccov = (
                    min(head_ccov, tail_ccov, s_path_ccov)
                    if s_path_ccov != 0.0
                    else min(head_ccov, tail_ccov)
                )
                concat_len = (
                    head_clen + tail_clen - overlap
                    if s_len == 0
                    else head_clen + s_len + tail_clen - overlap * 2
                )
                print(
                    "coverage: head contig eflow: ",
                    head_ccov,
                    " s path eflow: ",
                    s_path_ccov,
                    " tail contig eflow: ",
    ...
    for (tail_cno, head_cno) in concat_dicision_s:
        ...
        if head_cno in used_contig:
            print(
                "contig {0} be used from previous concatenation, skip".format(head_cno)
            )
            continue
        if tail_cno in used_contig:
            print(
                "contig {0} be used from previous concatenation, skip".format(tail_cno)
            )
            continue
        ...
        if is_linear:
            ...
            concat_ccov = min(
                v for v in [head_ccov, tail_ccov, s_path_ccov_l] if v > 0.0
            )
            ...
            concat_clen = (
                tail_clen + s_len_l + head_clen + s_len_r - overlap_count * overlap
            )
            concat_ccov = min(
                v
                for v in [head_ccov, tail_ccov, s_path_ccov_l, s_path_ccov_r]
                if v > 0.0
            )
        ...
        if head_ccov - concat_ccov < min_cov:
            ...
            temp_contigs_dict[head_cno][2] = (
                temp_contigs_dict[head_cno][2] - concat_ccov
            )

        if tail_ccov - concat_ccov < min_cov:
            used_contig.add(tail_cno)
        else:
            temp_contigs_dict[tail_cno][2] = (
                temp_contigs_dict[tail_cno][2] - concat_ccov
            )
    ...
    for cno, [contig, clen, ccov] in temp_contigs_dict.items():
        if cno not in used_contig:
            if clen >= min_len:
                print(
                    "Full len contig directly store into strain dict", cno, clen, ccov
                )
                used_contig.add(cno)
    ...
    for cno, [c, _, _] in concat_contig_dict.items():
        for n in c:
            if n not in node_to_contig_dict:
                node_to_contig_dict[n] = [
                    {cno},
                    graph.vp.dp[simp_node_dict[n]],
                    simp_node_dict[n],
                ]
            else:
        ...
        for i in range(len(c)):
            ...
            if c_i_1 != None:
                if (c_i, c_i_1) not in edge_to_contig_dict:
                    edge_to_contig_dict[(c_i, c_i_1)] = [
                        {cno},
                        graph.ep.flow[simp_edge_dict[(c_i, c_i_1)]],
                        simp_edge_dict[(c_i, c_i_1)],
                    ]
                else:
    ...
    for (u, v), [cnos, _, _] in list(edge_to_contig_dict.items()):
        for cno in used_contig:
            if cno in cnos:
                ...
                if DEBUG_MODE:
                    print(
                        "Remove used contig cno {0} from edge: {1} cno list".format(
                            cno, (u, v)
                        )
                    )
        for cno in concat_strain_dict.keys():
            if cno in cnos:
                ...
                if DEBUG_MODE:
                    print(
                        "Remov cand strain cno {0} from edge: {1} cno list".format(
                            cno, (u, v)
                        )
                    )
    ...
    for no, [cnos, _, _] in list(node_to_contig_dict.items()):
        for cno in used_contig:
            if cno in cnos:
                ...
                if DEBUG_MODE:
                    print(
                        "Remove used contig cno {0} from node: {1} cno list".format(
                            cno, no
                        )
                    )
        for cno in concat_strain_dict.keys():
            if cno in cnos:
                ...
                if DEBUG_MODE:
                    print(
                        "Remove cand strain cno {0} from node: {1} cno list".format(
                            cno, no
                        )
                    )
    ...
    for cno, (c, clen, ccov) in concat_strain_dict.items():
        contig_reduction(
            graph, c, cno, clen, ccov, simp_node_dict, simp_edge_dict, min_cov
        )
    ...
    for cno, [contig, clen, ccov] in concat_contig_dict.items():
        ...
        print_contig(
            ...
            ccov,
            contig,
            "partial length residue contig found after concatenation",
        )
...
def path_extraction(
    ...
    for gno, group in groups.items():
        ...
        if len(group) < 2:
            ...
            if len(group) == 1:
                ...
                # ratio%
                usage_ratio = round(
                    (node_usage_dict[id][0] / node_usage_dict[id][1]) * 100, 2
                )
        ...
        for src in srcs:
            for sink in sinks:
                print("Path extraction")
                p, plen = distance_search(
                    graph, simp_node_dict, node_usage_dict, [], src, [], sink, overlap
                )
                if p != None:
                    ...
                    plen = (
                        plen
                        + len(graph.vp.seq[simp_node_dict[src]])
                        + len(graph.vp.seq[simp_node_dict[sink]])
                        - 2 * overlap
                    )
    ...
    for gno, paths in paths_per_group_dict.items():
        ...
        for p_ids, plen in paths:
            ...
            pcov = numpy.mean(
                [graph.vp.dp[simp_node_dict[u]] for u in p_ids if u in simp_node_dict]
            )
...
def contig_reduction(
    ...
    for node in contig:
        ...
        e = (
            simp_edge_dict[(node, adj_node)]
            if (node, adj_node) in simp_edge_dict
            else None
        )
        ...
        # update edges
        if (graph.vp.color[u] == "gray" or graph.vp.color[v] == "gray") and (
            node,
            adj_node,
        ) in simp_edge_dict:
            graph.ep.flow[e] = 0
...
def optim_path_linear(
    ...
    """
    print(
        "Start path finding: {0} -> {1}, cand cov: {2}, bound: {3}".format(
            srccno, tgtcno, cand_cov, bound_length
        )
    )
    path = [src]
    pathqueue = deque()
        ...
        if path_len(graph, curr_path[1:-1], overlap) > bound_length:
            print(
                path_to_id_string(
                    graph,
                    curr_path[1:-1],
                    "curr path len overbound, skip impossible path: ",
                )
            )
            continue
    ...
    for p in rtn_paths:
        ...
        if not is_circular:
            total_len = get_concat_len(
                srccno,
                contig_dict[srccno][1],
                tgtcno,
                contig_dict[tgtcno][1],
                plen,
                overlap,
            )
        else:
        ...
        for node in p:
            num_similarity = (
                num_similarity + 1
                if abs(graph.vp.udp[node] - cand_cov) < threshold
                else num_similarity
            )
        print(
            "total len: ",
            ...
            " similarity: ",
            num_similarity,
            path_to_id_string(graph, p, "--->path: "),
        )
...
def contig_pairwise_concatenation(
    graph: Graph,
    simp_node_dict: dict,
    simp_edge_dict: dict,
    contig_dict: dict,
    ...
    def contig_pair_reduction(
        cno1, cno2, cand_cov, cand_path, cand_len, cno_mapping: dict
    ):
        """
        ...
        if (
            cliq_graph.vp.ccov[cnode1] <= threshold
            and cliq_graph.vp.ccov[cnode2] <= threshold
        ):
            print(
                "both node be used up, merge L1 to L2, keep the L1 in edges and L2 out edges only"
            )
            ...
            contig_dict[cno_merged] = [
                prev1[0] + [graph.vp.id[v] for v in cand_path] + prev2[0],
                cand_len,
                cand_cov,
            ]

            cnode_merged = cliq_graph_add_node(
                cliq_graph,
                cliq_node_dict,
                cno_merged,
                cand_len,
            ...
            for edge1out in cnode1.out_edges():
                cliq_graph_remove_edge(
                    cliq_graph,
                    cliq_edge_dict,
                    cliq_graph.vp.cno[edge1out.source()],
                    cliq_graph.vp.cno[edge1out.target()],
                    edge1out,
                )
            ...
            for edge1in in cnode1.in_edges():
                ...
                cliq_graph_remove_edge(
                    ...
                    cliq_edge_dict,
                    src1cno,
                    cliq_graph.vp.cno[edge1in.target()],
                    edge1in,
                )
                ...
                if src1cno != cno_merged:
                    cliq_graph_add_edge(
                        ...
                        cno_merged,
                        cnode_merged,
                        cliq_graph.ep.slen[edge1in],
                        cliq_graph.ep.text[edge1in],
                    )
            ...
            for edge2in in cnode2.in_edges():
                ...
                cliq_graph_remove_edge(
                    cliq_graph,
                    cliq_edge_dict,
                    cliq_graph.vp.cno[edge2in.source()],
                    cliq_graph.vp.cno[edge2in.target()],
                    edge2in,
                )
            ...
            for edge2out in cnode2.out_edges():
                ...
                cliq_graph_remove_edge(
                    cliq_graph,
                    cliq_edge_dict,
                    cliq_graph.vp.cno[edge2out.source()],
                    tgt2cno,
                    edge2out,
                )
                if cno_merged != tgt2cno:
                    cliq_graph_add_edge(
                        ...
                        tgt2cno,
                        tgt2,
                        cliq_graph.ep.slen[edge2out],
                        cliq_graph.ep.text[edge2out],
                    )
            ...
            prev1 = contig_dict.pop(cno1)
            contig_dict[cno_dup] = [
                prev1[0] + [graph.vp.id[v] for v in cand_path] + contig_dict[cno2][0],
                cand_len,
                cand_cov,
            ]
            contig_dict[cno2][2] -= cand_cov

            cnode_dup = cliq_graph_add_node(
                cliq_graph,
                cliq_node_dict,
                cno_dup,
                cand_len,
            ...
            for edge1 in cnode1.all_edges():
                ...
                cliq_graph_remove_edge(
                    cliq_graph,
                    cliq_edge_dict,
                    cliq_graph.vp.cno[edge1.source()],
                    cliq_graph.vp.cno[edge1.target()],
                    edge1,
                )
            ...
            prev2 = contig_dict.pop(cno2)
            contig_dict[cno_dup] = [
                contig_dict[cno1][0] + [graph.vp.id[v] for v in cand_path] + prev2[0],
                cand_len,
                cand_cov,
            ]
            ...
            for edge2 in cnode2.in_edges():
                ...
                cliq_graph_remove_edge(
                    cliq_graph,
                    cliq_edge_dict,
                    cliq_graph.vp.cno[edge2.source()],
                    cliq_graph.vp.cno[edge2.target()],
                    edge2,
                )
            ...
            print(
                "error: L1 node: ",
                cliq_graph.vp.ccov[cnode1],
                ", L2 node: ",
                cliq_graph.vp.ccov[cnode2],
            )
    ...
    def buffer_concatenation(concat_buffer: list, cno_mapping: dict):
        for (cno1, cno2, cov, delta) in concat_buffer:
            ...
            print(
                "-->Before mapping: {0}: {1} - {2}: {3}".format(
                    cno1, cno_mapping[cno1], cno2, cno_mapping[cno2]
                )
            )
            ...
            if not pairs:
                print(
                    "contig has been used from previous step: {0} {1}".format(
                        cno1, cno2
                    )
                )
                continue
            ...
            cno1m, cno2m = min(
                pairs,
                key=lambda p: pow(cliq_graph.vp.ccov[cliq_node_dict[p[0]]] - cov, 2)
                + pow(cliq_graph.vp.ccov[cliq_node_dict[p[1]]] - cov, 2),
            )

            print(
                "-->PAIR UP {0} - {1}, cov: {2}, diff: {3}".format(
                    cno1m, cno2m, cov, delta
                )
            )
            src = simp_node_dict[contig_dict[cno1m][0][-1]]
            tgt = simp_node_dict[contig_dict[cno2m][0][0]]
            cand_path, plen = dijkstra_sp(
                graph, simp_node_dict, src, tgt, cov, threshold, overlap
            )
            cand_len = get_concat_len(
                cno1m,
                contig_dict[cno1m][1],
                cno2m,
                contig_dict[cno2m][1],
            ...
            if len(list(contig_node.all_edges())) > 2:
                if cliq_graph.vp.clen[contig_node] < min_len:
                    ...
                    cliq_graph_remove_edge(
                        ...
                        cno,
                        cno,
                        cliq_graph.edge(contig_node, contig_node),
                    )
                    print(
                        "remove self edge+outer connection {0} -> {0} with node length < minlen".format(
                            cno
                        )
                    )
            else:
                ...
                cand_len = get_concat_len(
                    cno, contig_dict[cno][1], cno, contig_dict[cno][1], plen, overlap
                )
                ...
                if cand_path != None and cand_len != None:
                    ...
                    print(
                        path_to_id_string(
                            graph,
                            [simp_node_dict[cno] for cno in contig_dict[cno][0]]
                            + cand_path,
                            "cov: {0}".format(cov),
                        )
                    )
                    ...
                    cliq_graph_remove_edge(
                        cliq_graph, cliq_edge_dict, cno, cno, cliq_edge_dict[(cno, cno)]
                    )
                else:
    ...
    for (cno1, cno2) in cliq_edge_dict.keys():
        print("---------------------------------------------------------------")
        delta = abs(
            cliq_graph.vp.ccov[cliq_node_dict[cno1]]
            - cliq_graph.vp.ccov[cliq_node_dict[cno2]]
        )
        cov = min(
            cliq_graph.vp.ccov[cliq_node_dict[cno1]],
            cliq_graph.vp.ccov[cliq_node_dict[cno2]],
        ...
        for cyc in cycles:
            ...
            for i in range(len(cyc)):
                ...
                if graph.ep.color[e] != "black":
                    ...
                    break
                all_edges[
                    (cliq_graph.vp.cno[u], cliq_graph.vp.cno[v])
                ] = cliq_graph.edge(u, v)
            if skip_cycle:
                continue
            removed_edge_tuple = min(
                all_edges.items(), key=lambda e_tuple: cliq_graph.ep.slen[e_tuple[1]]
            )
            print(
                "Removed edge: ",
                removed_edge_tuple[0][0],
                "->",
                removed_edge_tuple[0][1],
            )
            cliq_graph_remove_edge(
                cliq_graph,
                cliq_edge_dict,
    ...
    while True:
        ...
        for no, contig_node in list(cliq_node_dict.items()):
            if cliq_graph.vp.color[contig_node] != "black":
                continue
            ind = len(
                [e for e in contig_node.in_edges() if cliq_graph.ep.color[e] == "black"]
            )
            outd = len(
                [
                    e
                    for e in contig_node.out_edges()
        ...
        for no, contig_node in list(L1_contigs.items()):
            out_edges = [
                e for e in contig_node.out_edges() if cliq_graph.ep.color[e] == "black"
            ]
        ...
        for (cno1, cno2), (node1, node2) in st_pairs.items():
            ...
            concat_buffer.append(
                (
                    cno1,
                    cno2,
                    min(cliq_graph.vp.ccov[node1], cliq_graph.vp.ccov[node2]),
                    delta,
                )
            )
    ...
    for cno, contig_node in list(cliq_node_dict.items()):
        ...
        if cno in contig_usage:
            ...
            if count > 0:
                print(
                    "original contig has been used, remove it to reduce the potential duplication"
                )
    ...
    for no, node in simp_node_dict.items():
        print(
            "Node: {0}, full: {1}, left: {2}, usage: {3}".format(
                no,
                round(graph.vp.dp[node], 2),
                round(graph.vp.udp[node], 2),
                round(
                    (graph.vp.dp[node] - graph.vp.udp[node]) * 100 / graph.vp.dp[node]
                ),
                2,
            )
        )
    ...
    for (a, b), bubble in bubbles.items():
        ...
        for var in bubble:
            ...
            if varid in node_to_contig_dict:
                ...
                print(
                    "    bin: {0}, bin size: {1}, curr usage: {2}%\n    strain: {3}\n".format(
                        varid,
                        graph.vp.dp[var],
                        pre_usages[varid],
                        [
                            (cno, contig_dict[cno][2])
                            for cno in node_to_contig_dict[varid]
                        ]
                        if varid in node_to_contig_dict
                        else "",
                ...
                print(
                    "    bin: {0}, bin size: {1}, curr usage: {2}%\n    strain: None\n".format(
                        varid, graph.vp.dp[var], pre_usages[varid]
                    )
                )
        if len(strains) > 0:
            bin_assignment, strain_placement = LPSolveBubbleLocal(
                bin, strains, threshold
            )
            for bin, contained_strain in bin_assignment.items():
                for cno in contained_strain:
                    curr_contig = contig_dict[cno][0]
                    rep_contig = contig_replacement(
                        curr_contig, prev_strain_loc[cno], bin
                    )
                    print(
                        "cno: {0} {1} --> {2}".format(
                            cno, list_to_string(curr_contig), list_to_string(rep_contig)
                        )
                    )
...
def a(contig_dict, args, THRESHOLD):
    while True:
        ...
        trivial_split_count, id_mapping = graph_split_trivial(
            grapha, simp_node_dicta, simp_edge_dicta, contig_dict
        )
        ...
        graph_to_gfa(
            ...
            simp_node_dicta,
            simp_edge_dicta,
            "{0}split_graph_L{1}1.gfa".format(TEMP_DIR, iterCount),
        )
        ...
        num_split, branch_id_mapping = graph_splitting(
            graphb, simp_node_dictb, simp_edge_dictb, contig_dict, THRESHOLD
        )
        # clean up graph
        graph_to_gfa(
            graphb,
            simp_node_dictb,
            simp_edge_dictb,
            "{0}split_graph_L{1}2.gfa".format(TEMP_DIR, iterCount),
        )
        ...
        contig_cov_fix(
            graphc, simp_node_dictc, simp_edge_dictc, contig_dict, branch_id_mapping
        )

        # simp path compactification
        simp_path_dict = simple_paths_to_dict(
            graphc, simp_node_dictc, simp_edge_dictc, args.overlap
        )
        ...
        graph_to_gfa(
            ...
            simp_node_dictc,
            simp_edge_dictc,
            "{0}split_graph_L{1}3.gfa".format(TEMP_DIR, iterCount),
        )
        graph5, simp_node_dict5, simp_edge_dict5 = flipped_gfa_to_graph(
            "{0}split_graph_L{1}3.gfa".format(TEMP_DIR, iterCount)
        )
        assign_edge_flow(graph5, simp_node_dict5, simp_edge_dict5)
        ...
        while currIndex < len(contig) - 1:
            ...
            if graph.vp.color[nnode] == "black":
                if is_splitter_branch(nnode):
                    ...
                    assert (prev_nid, nid) in simp_edge_dict and (
                        nid,
                        next_nid,
                    ) in simp_edge_dict
                    ...
                    involved_contig = (
                        prev_sp_contigs.intersection(curr_sp_contigs)
                    ).intersection(next_sp_contigs)
                    left_edge = simp_edge_dict[(prev_nid, nid)]
                    right_edge = simp_edge_dict[(nid, next_nid)]
                    print(
                        "prev: {0}, edge flow: {1}, curr: {2}, edge flow: {3}, next: {4}".format(
                            prev_nid,
                            graph.ep.flow[left_edge],
                            nid,
                            graph.ep.flow[right_edge],
                            next_nid,
                        )
                    )
...
def extract_cand_path(
    ...
    print(
        "-----------------------GRAPH BRANCH SPLIT & COMPACTIFICATION-------------------------------"
    )
    grapha = graph3
    ...
    while True:
        ...
        # simp path compactification
        simp_path_compactification(
            graphc, simp_node_dictc, simp_edge_dictc, contig_dict, args.overlap
        )
    ...
    print(
        ...
        total_removed_branch_nt,
        " total trivial branches removed: ",
        total_removed_branch_t,
    )
    ...
    contig_dict_to_fasta(
        ...
        simp_node_dicta,
        args.overlap,
        "{0}pre_contigs.fasta".format(TEMP_DIR),
    )
    minimap_api(
        args.ref_file,
        "{0}pre_contigs.fasta".format(TEMP_DIR),
        "{0}pre_contigs_to_strain.paf".format(TEMP_DIR),
    )

    grapha, simp_node_dicta, simp_edge_dicta, contig_dict = reindexing(
        grapha, simp_node_dicta, simp_edge_dicta, contig_dict
    )
    ...
    contig_dict_to_fasta(
        ...
        simp_node_dicta,
        args.overlap,
        "{0}post_contigs.fasta".format(TEMP_DIR),
    )
    minimap_api(
        args.ref_file,
        "{0}post_contigs.fasta".format(TEMP_DIR),
        "{0}post_contigs_to_strain.paf".format(TEMP_DIR),
    )
    ...
    graph_to_gfa(
        ...
        simp_node_dicta,
        simp_edge_dicta,
        "{0}rbsdt_graph_L5.gfa".format(TEMP_DIR),
    )
    graph5, simp_node_dict5, simp_edge_dict5 = flipped_gfa_to_graph(
        "{0}rbsdt_graph_L5.gfa".format(TEMP_DIR)
    )
    ...
    if args.ref_file:
        map_ref_to_graph(
            ...
            "{0}rbsdt_graph_L5.gfa".format(TEMP_DIR),
            True,
            "{0}node_to_ref_red.paf".format(TEMP_DIR),
            "{0}temp_gfa_to_fasta.fasta".format(TEMP_DIR),
        )
    gfa_to_fasta(
        "{0}rbsdt_graph_L5.gfa".format(TEMP_DIR),
        "{0}graph_L5_node.fasta".format(TEMP_DIR),
    )
    ...
    graph_to_gfa(
        ...
        simp_node_dict1,
        simp_edge_dict1,
        "{0}t_graph_L1_dag.gfa".format(TEMP_DIR),
    )
...
def branch_split_formal(
    graph: Graph, simp_node_dict: dict, simp_edge_dict: dict, threshold
):
    def is_partial(e):
...
# cliq graph version
def allowed_concat_init(
    graph: Graph, contig_dict: dict, simp_node_dict: dict, max_len, threshold, overlap
):
    """
    ...
    for tail_cno, [tail_contig, tail_clen, tail_ccov] in contig_dict.items():
        for head_cno, [head_contig, head_clen, head_ccov] in contig_dict.items():
            ...
            status = None
            if tail_cno != head_cno:
                isParallel, intersects, cend, status = check_contig_intersection(
                    tail_cno, tail_contig, head_cno, head_contig
                )
                if isParallel:
                    impossible_concat_dict[tail_cno].add(head_cno)
                if status != None:
            ...
            if head_cno not in impossible_concat_dict[tail_cno]:
                ...
                if intersects != None and cend != None:
                    ...
                    intersect_len = sum(
                        [len(graph.vp.seq[simp_node_dict[id]]) for id in intersects]
                    ) - overlap * (len(intersects) + cend)
                    total_len = -intersect_len
                sp, plen, pmark = dijkstra_sp(
                    graph,
                    tail_node,
                    head_node,
                    min(tail_ccov, head_ccov),
                    threshold,
                    overlap,
                )
                if sp != None:
...
def contig_clique_graph_build(
    ...
    for cno, (contig, clen, ccov) in contig_dict.items():
        ...
        cliq_graph.vp.text[contig_node] = (
            cno + ":" + str(clen) + ":" + str(round(ccov, 2))
        )
        cliq_node_dict[cno] = contig_node

    concat_plan, sp_path_dict = allowed_concat_init(
        graph, contig_dict, simp_node_dict, max_len, threshold, overlap
    )

    cno2cno_adjMtx_csv = []
    cno2cno_adjMtx_csv.append(
        [cno + "/" + str(round(contig_dict[cno][2])) for cno in concat_plan.keys()]
    )
    ...
    for tail_cno, head_cnos in concat_plan.items():
        print(
            "------> tail cno: ",
            tail_cno,
            " can concat with following head cnos: ",
            head_cnos,
        )
        ...
        for head_cno in head_cnos:
            plen = (
                int(sp_path_dict[(tail_cno, head_cno)][1])
                if sp_path_dict[(tail_cno, head_cno)] != None
                else -1
            )
            abs_cdif = abs(contig_dict[tail_cno][2] - contig_dict[head_cno][2])
            curr_row_csv[cno_to_index_csv[head_cno]] = (
                str(round(abs_cdif)) + ":" + str(plen)
            )
            ...
            if color != "X":
                ...
                if color != "B":
                    ...
                    if abs_diff < threshold:
                        ...
                        cliq_graph.ep.color[e] = "gray"
                    print(
                        "Decided Edge: {0} -> {1}, asb_diff: {2}".format(
                            index_to_cno[rowId], index_to_cno[colId], abs_diff
                        )
                    )
        ...
        for (cno1, cno2, cov, delta) in concat_buffer:
            ...
            if (cno1m.split("->")[-1], cno2m.split("->")[0]) in sp_path_dict:
                ...
                if sp_path_dict[(cno1m_rel, cno2m_rel)] != None:
                    ...
                    intersect = set(contig_dict[cno1m][0]).intersection(
                        set(contig_dict[cno2m][0])
                    )
                    print(
                        "{0} -> {1} end to end intersects: {2}".format(
                            cno1m, cno2m, intersect
                        )
                    )
                    ...
                    # in order
                    cand_path = [
                        simp_node_dict[node_id]
                        for i, node_id in enumerate(contig_dict[cno1m][0])
                        if intermediate_nodes_index[i]
                    ]
                    ...
                    clen1m = path_len(
                        graph,
                        [simp_node_dict[node_id] for node_id in contig1m],
                        overlap,
                    )
                    ...
                    clen2m = path_len(
                        graph,
                        [simp_node_dict[node_id] for node_id in contig2m],
                        overlap,
                    )
                    contig_dict[cno2m] = [contig2m, clen2m, ccov2m]
                    print(
                        "overlap contig concat: ",
                        cno1m,
                        contig1m,
                        cno2m,
                        contig2m,
                ...
                src = simp_node_dict[contig_dict[cno1m][0][-1]]
                tgt = simp_node_dict[contig_dict[cno2m][0][0]]
                cand_path, plen, pmark = dijkstra_sp(
                    graph, src, tgt, cov, threshold, overlap
                )
            cand_len = get_concat_len(
                cno1m,
                contig_dict[cno1m][1],
    ...
    for cno, contig_node in list(cliq_node_dict.items()):
        if contig_node in list(contig_node.out_neighbors()):
            if len(list(contig_node.all_edges())) > 2:
                ...
                print(
                    "should be removed already ... remove self edge+outer connection {0} -> {0}, legacy".format(
                        cno
                    )
                )
            else:
