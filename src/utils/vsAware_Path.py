...
def st_path(
    ...
    sp_vlist = []
    try:
        sp_vlist, _ = shortest_path(
            graph, s, t, graph.ep.eval, negative_weights=True, dag=is_dag
        )
        ...
        sp_vlist, _ = shortest_path(
            graph, s, t, graph.ep.eval, negative_weights=False, dag=is_dag
        )
    return sp_vlist
...
def extract_cand_path(
    ...
    for cno, [contig, clen, ccov] in list(contig_dict.items()):
        if (
            ("global_src", contig[0]) in simp_edge_dict
            and (contig[-1], "global_sink") in simp_edge_dict
    ...
    while len(contig_dict.keys()) > 0:
        ...
        if reachable(
            graph, simp_node_dict, simp_node_dict[contig[-1]], simp_node_dict[contig[0]]
        ):
            ...
            logger.debug(
                "found path: " + list_to_string([graph.vp.id[n] for n in sp_vlist])
            )
            ...
            sphead_vlist = st_path(
                graph,
                ccov,
                global_src,
                simp_node_dict[contig[0]],
                b0,
                b1,
                logger,
                is_dag,
            )
            sptail_vlist = st_path(
                graph,
                ccov,
                simp_node_dict[contig[-1]],
                global_sink,
                b0,
                b1,
                logger,
        ...
        score = []
        for flow in contig_flow(
            graph, simp_edge_dict, [graph.vp.id[n] for n in strain]
        ):
        ...
        logger.debug(
            path_to_id_string(
                graph, strain, "strain, len: {0}, ccov: {1}".format(plen, ccov)
            )
        )
