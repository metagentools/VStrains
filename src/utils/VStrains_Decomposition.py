...
def cov_split(us: list, ws: list, pe_info: dict, sec_comb: list, kept_link: dict, in_usage: dict, in_capacity: dict, out_usage: dict, out_capacity: dict, logger):
    ...
    for uid in us:
        ...
        if opt_us[0] == uid and out_usage[wid] == 0 and (uid, wid) not in kept_link:
            ...
            logger.debug("Found coverage best match: {0} -> {1} with cov: {2}, {3}, checking delta bound: {4}".
                format(uid, wid, in_capacity[uid], out_capacity[wid], delta))
            if abs(in_capacity[opt_us[1]] - out_capacity[wid]) <= delta or abs(in_capacity[uid] - out_capacity[opt_ws[1]]) <= delta:
                logger.debug("ambiguous matching, skip")
            else:
                logger.debug("added")
                in_usage[uid] += 1
                out_usage[wid] += 1
                kept_link[(uid, wid)] = ((in_capacity[uid] + out_capacity[wid]) / 2, 
...
def balance_split(graph: Graph, simp_node_dict: dict, simp_edge_dict: dict, contig_dict: dict, pe_info: dict, logger: Logger, ref_file: str, temp_dir: str, count_id: int, threshold, is_prim: bool):
    ...
    for no, node in non_trivial_branches.items():
        ...
        if any([pe_info[(uid,uid)] == None for uid in us]) or any([pe_info[(wid,wid)] == None for wid in ws]):
            logger.debug("current non-trivial branch: {0} is related to current iteration, split later".format(no))
            continue
        if not is_non_trivial(graph, node):
            logger.debug("current non-trivial branch: {0} is not non-trivial, potential bug".format(no))
            continue
        if len(us) != len(ws):
        ...
        for id in us+ws:
            ...
            if all([single.count("*") > 0 for single in singles]):
                logger.debug("leaf:{0} is total branch nodes, no link information, skip link split".format(id))
                split_via_link = False
                break

        # not perform link-split if no combination has link information
        if all([pe_info[(min(uid, wid), max(uid, wid))] == 0 for uid in us for wid in ws]):
            logger.debug("current branch node too long, no link information, skip link split")
            split_via_link = False
        ...
        if ref_file:
            ...
            for uid in us:
                for wid in ws:
                    ...
                    best_ref_l = [ref for [_, l, ref, nm] in ref_l if nm == 0 and l == len(graph.vp.seq[u])]
                    ref_r = best_aln_score(graph, "R", [w], ref_file, temp_dir)
                    best_ref_r = [ref for [_, l, ref, nm] in ref_r if nm == 0 and l == len(graph.vp.seq[w])]
                    lrefs = lrefs.union(best_ref_l)
                    rrefs = rrefs.union(best_ref_r)
                    ref_pair_dict[(uid, wid)] = set(best_ref_l).intersection(set(best_ref_r))
                    ref_all_dict[(uid, wid)] = set([ref for [_, _, ref, nm] in ref_l if nm < 5]).union(set([ref for [_, _, ref, nm] in ref_r if nm < 5]))
            ...
            if len(sym_diff) > 0:
                logger.debug("Current branch have force mismatch connection for following strains: {0}".format(sym_diff))
        # debug only
        ...
        for uid in us:
            for wid in ws:
                ...
                if ref_file:
                    logger.debug("intersect reference: {0}".format(ref_pair_dict[(uid, wid)]))
                ...
                if accept:
                    in_usage[uid] += 1
                    out_usage[wid] += 1
                    kept_link[(uid, wid)] = ((in_capacity[uid] + out_capacity[wid]) / 2, curr_pe)
                else:
                    logger.debug("current link is secondary choice, process later")
                    sec_comb.append((uid, wid, curr_pe))
        if is_prim:
            if split_via_link:
                link_split(sec_comb, kept_link, 
                    in_usage, in_capacity, out_usage, out_capacity, logger)
        else:
            # secondary split, via link first, then coverage
            cov_split(us, ws, pe_info, sec_comb, kept_link, 
                in_usage, in_capacity, out_usage, out_capacity, logger)
        if not(all([u==1 for u in in_usage.values()]) and all([v==1 for v in out_usage.values()])):
            logger.debug("->Not satisfy N-N split, skip: {0}".format(kept_link))
            continue
        worst_pair_diff = max([abs(in_capacity[uid] - out_capacity[wid]) for (uid,wid) in kept_link.keys()])
        if worst_pair_diff > 4 * threshold:
            logger.debug("worst pair coverage diff greater than 4 delta: {0} > {1}, too uneven, skip: {2}".format(worst_pair_diff, 4* threshold, kept_link))
            continue
        ...
        for (uid, wid), (sub_flow, pe) in kept_link.items():
            ...
            if ref_file:
                if len(ref_pair_dict[(uid, wid)]) != 0:
                    ...
                    if pe <= cut:
                        ...
                        if pe < 5:
                            logger.debug("correct node with 0 pest {0}->{1}->{2}, with branch size: {3}".format(uid, no, wid, len(graph.vp.seq[node])))
                else:
                    is_graph_error = False
                    if uid in error_nos:
                        logger.debug("src: {0} is incorrect graph erroroness node, no optimal ref".format(uid))
                        is_graph_error = True
                    if wid in error_nos:
                        logger.debug("tgt: {0} is incorrect graph erroroness node, no optimal ref".format(wid))
                        is_graph_error = True
            ...
            counter += 1
            sub_node = graph_add_vertex(graph, simp_node_dict, sub_id, sub_flow, graph.vp.seq[node])
        ...
        for cno in support_contigs:
            ...
            wid = curr_contig[branch_ind + 1] if branch_ind < len(curr_contig) - 1 else None
            if uid != None and wid != None:
                ...
                for sub_id in link2subs.values():
                    ...
                    contig_dict[cno+"$"+str(sub_id.split("*")[-1])] = [[sub_id], len(graph.vp.seq[simp_node_dict[sub_id]]), graph.vp.dp[simp_node_dict[sub_id]]]
            elif uid != None and wid == None:
                for (uid2, _), sub_id in link2subs.items():
                    if uid == uid2:
                        curr_contig[branch_ind] = sub_id
                        contig_dict[cno+"$"+str(sub_id.split("*")[-1])] = [list(curr_contig), clen, ccov]
            else:
    ...
    if ref_file:
        ...
        ax1.scatter(false_error_X, false_error_Y, color="blue", s=100, label="False-Positive")
        ...
        plt.savefig("{0}{1}".format(temp_dir, "/tmp/scatter_plot_pest_{0}.png".format(count_id)))

    return len(set(split_branches))

def trivial_split(graph: Graph, simp_node_dict: dict, simp_edge_dict: dict, pe_info: dict, logger: Logger):
    """
        ...
        for inode in set(ntnode.in_neighbors()):
            ...
            if len(ines) > 1 and len(outes) == 1:
                ...
                for i in range(len(ines)):
                    ...
                    for nno in simp_node_dict.keys():
                        pe_info[(min(graph.vp.id[snode], nno), max(graph.vp.id[snode], nno))] = None
...
def edge_cleaning(graph: Graph, simp_edge_dict: dict, contig_dict: dict, pe_info: dict, logger: Logger):
    ...
    assigned = dict.fromkeys([(graph.vp.id[e.source()], graph.vp.id[e.target()]) for e in graph.edges()], False)
    ...
    while True:      
        for node in graph.vertices():
            ...
            if in_d == 1:
                assigned[(graph.vp.id[in_e[0].source()], graph.vp.id[in_e[0].target()])] = True
                un_assigned_edge = un_assigned_edge - 1
            if out_d == 1:
                assigned[(graph.vp.id[out_e[0].source()], graph.vp.id[out_e[0].target()])] = True
                un_assigned_edge = un_assigned_edge - 1
    ...
    logger.debug("un-assigned edges after node-weight coverage iteration : {0}".format(un_assigned_edge))
    for (u,v) in assigned.keys():
        if not assigned[(u,v)]:
            logger.debug("***cross un-assigned edge: {0} -> {1}, with paired end link {2}".format(u,v, pe_info[(min(u,v), max(u,v))]))
            if (u,v) in edge_to_contig_dict:
                logger.debug("support contig: {0}, force assign".format(edge_to_contig_dict[(u,v)]))
                assigned[(u,v)] = True
            else:
    ...
    for (u,v) in assigned.keys():
        if not assigned[(u,v)]:
            ...
            if not force_assign:
                ...
                logger.debug("intersect unsupported edge: {0} -> {1}, removed".format(u,v))
            else:
...
def iter_graph_disentanglement(graph: Graph, simp_node_dict: dict, simp_edge_dict: dict, contig_dict: dict, pe_info: dict, ref_file: str, logger: Logger, threshold, temp_dir):
        ...
        while it < BOUND_ITER:
            ...
            if num_split > 0:
                ...
                if do_trivial_split:
                    ...
                    trivial_split_count, id_mapping = trivial_split(graph, simp_node_dict, simp_edge_dict, pe_info, logger)
    ...
    logger.debug(
        ...
        + str(total_removed_branch)
    )
    non_trivial_branches = get_non_trivial_branches(graph, simp_node_dict)
    logger.debug(list_to_string(non_trivial_branches.keys(), "non-trivial branches ({0}) left after paired-end&single-strand links".format(len(non_trivial_branches))))
