...
from utils.vsAware_CovBalance import (
    assign_edge_flow, 
...
def run(args, logger):
    ...
    graph_to_gfa(graph, simp_node_dict, simp_edge_dict, logger, "{0}/gfa/graph_L0.gfa".format(TEMP_DIR))
    graph0, simp_node_dict0, simp_edge_dict0 = flipped_gfa_to_graph("{0}/gfa/graph_L0.gfa".format(TEMP_DIR), logger)
    graph0, simp_node_dict0, simp_edge_dict0, idx_mapping = reindexing(graph0, simp_node_dict0, simp_edge_dict0)

    contig_dict, contig_info = spades_paths_parser(graph0, simp_node_dict0, simp_edge_dict0, idx_mapping, logger, args.path_file, args.min_len)
    copy_contig_dict = {}
    ...
    graph_to_gfa(graph0, simp_node_dict0, simp_edge_dict0, logger, "{0}/gfa/t_graph_L1.gfa".format(TEMP_DIR))
    graph1, simp_node_dict1, simp_edge_dict1 = flipped_gfa_to_graph("{0}/gfa/t_graph_L1.gfa".format(TEMP_DIR), logger)
    ...
    graph_simplification(graph1, simp_node_dict1, simp_edge_dict1, contig_dict, logger, THRESHOLD)
    graph_to_gfa(graph1, simp_node_dict1, simp_edge_dict1, logger, "{0}/gfa/st_graph_L2.gfa".format(TEMP_DIR))
    graph2, simp_node_dict2, simp_edge_dict2 = flipped_gfa_to_graph("{0}/gfa/st_graph_L2.gfa".format(TEMP_DIR), logger)

    coverage_rebalance_s(graph2, simp_node_dict2, simp_edge_dict2, logger)
    graph_to_gfa(graph2, simp_node_dict2, simp_edge_dict2, logger, "{0}/gfa/cst_graph_L3.gfa".format(TEMP_DIR))
    graph3, simp_node_dict3, simp_edge_dict3 = flipped_gfa_to_graph("{0}/gfa/cst_graph_L3.gfa".format(TEMP_DIR), logger)
    ...
    # stat evaluation
    if args.ref_file:
        map_ref_to_graph(args.ref_file, simp_node_dict3, "{0}/gfa/cst_graph_L3.gfa".format(TEMP_DIR), 
        False, "{0}/paf/node_to_ref.paf".format(TEMP_DIR), "{0}/tmp/temp_gfa_to_fasta_pre.fasta".format(TEMP_DIR))
    ...
    graphf, simp_node_dictf, simp_edge_dictf = iterated_graph_split(graph3, simp_node_dict3, simp_edge_dict3, contig_dict, logger, TEMP_DIR, b0, b1, THRESHOLD)
    ...
    # stat evaluation
    if args.ref_file:
        map_ref_to_graph(args.ref_file, simp_node_dictf, "{0}/gfa/rbsdt_graph_L5.gfa".format(TEMP_DIR), 
        False, "{0}/paf/node_to_ref_red.paf".format(TEMP_DIR), "{0}/tmp/temp_gfa_to_fasta.fasta".format(TEMP_DIR))
    ...
    strain_dict = extract_cand_path(graphf, simp_node_dictf, simp_edge_dictf, contig_dict, logger, b0, b1, THRESHOLD)
    ...
    strain_repeat_resol(graph0, simp_node_dict0, strain_dict, contig_info, copy_contig_dict, logger)
