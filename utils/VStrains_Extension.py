    ...
    while curr != None and not visited[graph.vp.id[curr]]:
        ...
        if len(out_branches) == 0:
            ...
            if graph.vp.id[curr] in full_link and len(final_path) > 1:
                ...
                curr_links = [
                    simp_node_dict[wid]
                    for (uid, wid) in full_link[graph.vp.id[curr]].keys()
                    if uid == graph.vp.id[final_path[-2]]
                ]
                if len(curr_links) == 1:
                    # curr = curr_links[0]
                    ...
                    if graph.vp.dp[curr_links[0]] - ccov <= -2 * threshold:
                        curr = None
                        logger.debug(
                            "{0} single link < 2delta, use coverage".format(
                                graph.vp.id[curr_links[0]]
                            )
                        )
                    else:
                        curr = curr_links[0]
            ...
            if f_assigned:
                ...
                if len(final_path) > 1 and len(in_branches) > 0:
                    ...
                    opt_ws = sorted(
                        out_branches,
                        key=lambda ww: abs(graph.vp.dp[curru] - graph.vp.dp[ww]),
                    )
                    bestw = opt_ws[0]
                    opt_us = sorted(
                        in_branches,
                        key=lambda uu: abs(graph.vp.dp[bestw] - graph.vp.dp[uu]),
                    )
                    if opt_us[0] == curru:
                        delta = max(
                            2 * abs(graph.vp.dp[curru] - graph.vp.dp[bestw]), threshold
                        )
                        if (
                            len(opt_us) > 1
                            and abs(graph.vp.dp[opt_us[1]] - graph.vp.dp[bestw])
                            <= delta
                        ):
                        ...
                        if (
                            len(opt_ws) > 1
                            and abs(graph.vp.dp[curru] - graph.vp.dp[opt_ws[1]])
                            <= delta
                        ):
                            logger.debug("ambiguous best matching, stop extension")
                            continue
            ...
            if curr == None:
                single_bests = sorted(
                    [(onode, graph.vp.dp[onode]) for onode in out_branches],
                    key=lambda tp: tp[1],
                    reverse=True,
                )
                logger.debug(
                    "Try last bit: 1st: {0}, 2nd: {1}, delta: {2}, cov: {3}".format(
                        (graph.vp.id[single_bests[0][0]], single_bests[0][1]),
                        (graph.vp.id[single_bests[1][0]], single_bests[1][1]),
                        threshold,
                        ccov,
                    )
                )
                if (
                    single_bests[0][1] - ccov > -threshold
                    and single_bests[1][1] - ccov <= -threshold
                ):
        ...
        if len(in_branches) == 0:
            ...
            if graph.vp.id[curr] in full_link and len(final_path) > 1:
                ...
                curr_links = [
                    simp_node_dict[uid]
                    for (uid, wid) in full_link[graph.vp.id[curr]].keys()
                    if wid == graph.vp.id[final_path[1]]
                ]
                if len(curr_links) == 1:
                    # curr = curr_links[0]
                    # logger.debug("single link next: {0}".format(graph.vp.id[curr]))
            ...
            if f_assigned:
                ...
                if len(final_path) > 1 and len(out_branches) > 0:
                    ...
                    opt_us = sorted(
                        in_branches,
                        key=lambda uu: abs(graph.vp.dp[currw] - graph.vp.dp[uu]),
                    )
                    bestu = opt_us[0]
                    opt_ws = sorted(
                        out_branches,
                        key=lambda ww: abs(graph.vp.dp[bestu] - graph.vp.dp[ww]),
                    )
                    if opt_ws[0] == currw:
                        delta = max(
                            2 * abs(graph.vp.dp[currw] - graph.vp.dp[bestu]), threshold
                        )
                        if (
                            len(opt_us) > 1
                            and abs(graph.vp.dp[opt_us[1]] - graph.vp.dp[currw])
                            <= delta
                        ):
                            logger.debug("ambiguous best matching, stop extension")
                            continue
                        if (
                            len(opt_ws) > 1
                            and abs(graph.vp.dp[bestu] - graph.vp.dp[opt_ws[1]])
                            <= delta
                        ):
                            logger.debug("ambiguous best matching, stop extension")
                            continue
            ...
            if curr == None:
                single_bests = sorted(
                    [(inode, graph.vp.dp[inode]) for inode in in_branches],
                    key=lambda tp: tp[1],
                    reverse=True,
                )
                logger.debug(
...
def final_extension(
    graph: Graph, simp_node_dict: dict, contig: list, full_link: dict, logger: Logger
):
    visited = dict.fromkeys(simp_node_dict.keys(), False)
    for no in contig[1:-1]:
        visited[no] = True
...
def path_extension(
    ...
    logger.debug(
        "-------------------------PATH Extension, delta: {0}".format(threshold)
    )
    ...
    while len(contig_dict) > 0:
                    ...
                    if len(red_id_mapping[uid]) == 1 or len(red_id_mapping[wid]) == 1:
                        for uuid in red_id_mapping[uid]:
                            for wwid in red_id_mapping[wid]:
                                if (
                                    (uuid, wwid) not in kept_link
                                    and (simp_node_dict[uuid] in node.in_neighbors())
                                    and (simp_node_dict[wwid] in node.out_neighbors())
                                ):
        ...
        if all(usages[cn] > 0 for cn in contig):
            print_contig(
                longest_cno, clen, ccov, contig, logger, "-----> Used previously"
            )
            continue
        if any(graph.vp.color[simp_node_dict[no]] == "gray" for no in contig):
            print_contig(
                ...
                contig,
                logger,
                "-----> Some node low cov, skip",
            )
            continue

        cbubbles = get_bubble_nodes(simp_node_dict, contig)
        bbl_cov = (
            numpy.median([graph.vp.dp[node] for node in cbubbles])
            if len(cbubbles) != 0
            else ccov
        )
        print_contig(
            ...
            contig,
            logger,
            "-----> Current extending contig: org ccov: {0}, use min {1}".format(
                ccov, min(ccov, bbl_cov)
            ),
        )

        path = contig_extension(
            graph,
            simp_node_dict,
            contig,
            min(ccov, bbl_cov),
        ...
        bbl_pcov = (
            numpy.median([graph.vp.dp[node] for node in pbubbles])
            if len(pbubbles) != 0
            else ccov
        )
        ...
        logger.debug(
            path_to_id_string(
                graph, path, "---*extended from contig {0}".format(longest_cno)
            )
        )
        logger.debug(
            "name: {0}, plen: {1}, pcov: {2}, bubble cov: {3}".format(
                pno, plen, pcov, bbl_pcov
            )
        )
    ...
    for _, sp_nodes in seq_dict.items():
        if len(sp_nodes) > 1:
            sorted_sp_nodes = sorted(
                sp_nodes, key=lambda vnode: graph.vp.dp[vnode], reverse=True
            )
    ...
    for no, node in nt_branches.items():
        ...
        for uid, wid, lf in sorted_comb:
            ...
            if lf > 0 and in_usage[uid] == 0 and out_usage[wid] == 0:
                logger.debug(
                    "-----SEC LINK {0} -> {1} LINK: {2}-----".format(uid, wid, lf)
                )
    ...
    for node in sorted(
        graph.vertices(), key=lambda nd: len(graph.vp.seq[nd]), reverse=True
    ):
        ...
        if usages[graph.vp.id[node]] == 0:
            ...
            path = final_extension(
                graph, simp_node_dict, [graph.vp.id[node]], final_links, logger
            )
            pno = "N" + str(rid)
            plen = path_len(graph, path)
            path_ids = [graph.vp.id[n] for n in path]
            ...
            pcov = (
                numpy.median([graph.vp.dp[node] for node in pbubbles])
                if len(pbubbles) != 0
                else graph.vp.dp[node]
            )
            logger.debug(
                path_to_id_string(
                    graph,
                    path,
                    "---*extended from free node {0}".format(graph.vp.id[node]),
                )
            )
